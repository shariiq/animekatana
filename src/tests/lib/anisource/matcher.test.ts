...
import {
  ...
  resolvePlayback,
} from '../../../lib/anisource/matcher';
import type { Anime } from '../../../lib/types';

const source = { id: 'source-one', name: 'Source One', base_url: 'https://example.test' };
...
describe('resolvePlayback', () => {
  ...
  it('caches a completed resolution for an identical request', async () => {
    const search = vi.spyOn(anisource, 'search').mockResolvedValue([
      { id: 'opaque-id', title: 'Test Anime', alternative_titles: [] },
    ]);
