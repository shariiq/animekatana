			...
			(
				{
					...
					CANCELLED: "Cancelled",
					HIATUS: "Ongoing",
				} as Record<string, Anime["status"]>
			)[String(media.status)] ?? "Not Yet Aired",
		format:
			(
				{
					TV: "TV",
					...
					ONA: "ONA",
					SPECIAL: "SPECIAL",
				} as Record<string, Anime["format"]>
			)[String(media.format)] ?? "SPECIAL",
	...
	return singleFlight.getOrSet(key, async () => {
		...
		if (!response.ok)
			throw new AniListError(
				response.status === 429
					? "AniList is rate-limiting requests. Please try again shortly."
...
function page(raw: RawPage): PaginatedResponse<Anime> {
	return {
		...
		hasNext: raw.pageInfo.hasNextPage,
...
export async function getHome(year: number, season: string) {
	...
	return {
		...
		seasonal: page(data.seasonal),
		upcoming: page(data.upcoming),
	};
}
...
export async function getCatalogue(input: {
	...
	const query = catalogueQuery(filters);
	const variables: Record<string, unknown> = {
		page: input.page ?? 1,
		perPage: 24,
		sort: input.sort ?? ["POPULARITY_DESC"],
	};
	if (input.search) variables.search = input.search;
	if (input.season) variables.season = input.season;
	if (input.year) variables.seasonYear = input.year;
	if (input.format) variables.format = input.format;
