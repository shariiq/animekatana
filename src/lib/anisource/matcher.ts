...
function aliases(anime: Anime, fallback: boolean) {
	const primary = [anime.title.english, anime.title.romaji].filter((title): title is string =>
		Boolean(title),
	);
	const later = [anime.title.native, ...anime.title.synonyms].filter((title): title is string =>
		Boolean(title),
	);
...
function scoreCandidate(anime: Anime, candidate: SourceAnime, query: string): number {
	const titles = [
		candidate.title,
		...(candidate.alternative_titles ?? candidate.alternativeTitles ?? []),
	];
	return Math.max(
		...titles.map((title) => similarity(query, title)),
		...aliases(anime, true).map((title) => similarity(title, candidate.title)),
	);
}
async function searchSource(
	...
	const responses = await Promise.allSettled(
		queries.map((query) => anisource.search(sourceId, query).then((items) => ({ query, items }))),
	);
...
export async function resolvePlayback(anime: Anime, preferredSource?: string): Promise<Resolution> {
	...
	if (!preferred)
		return { state: "unavailable", reason: "No playback sources are currently available." };
	...
	if (
		firstBest &&
		firstBest.confidence >= 0.9 &&
		(!firstRunnerUp || firstBest.confidence - firstRunnerUp.confidence >= 0.08)
	) {
		const match: Resolution = { state: "matched", ...firstBest };
		results.set(cacheKey, match, 86_400);
		return match;
	}
	...
	const expanded = await Promise.all([
		searchSource(anime, preferred.id, fallbackQueries),
		...fallbackSources.map((source) => searchSource(anime, source.id, fallbackQueries)),
	]);
	const candidates = [
		...new Map(
			expanded.flat().map((candidate) => [`${candidate.sourceId}:${candidate.animeId}`, candidate]),
		).values(),
	...
	const outcome: Resolution = candidates.filter((candidate) => candidate.confidence >= 0.68).length
		? {
				state: "ambiguous",
				candidates: candidates.filter((candidate) => candidate.confidence >= 0.68).slice(0, 5),
			}
		: { state: "unavailable", reason: "No safe playback match was found for this title." };
	results.set(cacheKey, outcome, 21_600);
	return outcome;
}
